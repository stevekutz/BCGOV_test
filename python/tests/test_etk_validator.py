...
class TestEtkValidator:
    ...
    def test_sample_data_event_issuance_passes_validation(self):
        sample_data = load_json_into_dict('python/tests/sample_data/event_issuance.json')
    ...
    def test_sample_data_vt_payment_passes_validation(self):
        sample_data = load_json_into_dict('python/tests/sample_data/vt_payment.json')
        assert type(sample_data) is dict
        validate_class = Validate(Config())
        assert validate_class.validate(sample_data)['isSuccess'] is True

    def test_sample_data_vt_query_passes_validation(self):
        sample_data = load_json_into_dict('python/tests/sample_data/vt_query.json')
        assert type(sample_data) is dict
        validate_class = Validate(Config())
        assert validate_class.validate(sample_data)['isSuccess'] is True

    def test_sample_data_vt_dispute_passes_validation(self):
        sample_data = load_json_into_dict('python/tests/sample_data/vt_dispute.json')
        assert type(sample_data) is dict
        validate_class = Validate(Config())
        assert validate_class.validate(sample_data)['isSuccess'] is True

    def test_sample_data_vt_dispute_status_update_passes_validation(self):
        sample_data = load_json_into_dict('python/tests/sample_data/vt_dispute_status_update.json')
        assert type(sample_data) is dict
        validate_class = Validate(Config())
        assert validate_class.validate(sample_data)['isSuccess'] is True

    def test_sample_data_vt_dispute_finding_passes_validation(self):
        sample_data = load_json_into_dict('python/tests/sample_data/vt_dispute_finding.json')
        assert type(sample_data) is dict
        validate_class = Validate(Config())
        assert validate_class.validate(sample_data)['isSuccess'] is True
    ...
    @staticmethod
    def test_message_returns_errors_from_most_relevant_schema():
        sample_data = load_json_into_dict('python/tests/sample_data/vt_payment.json')
        sample_data['event_type'] = "bad_type"
        validate_class = Validate(Config())
        assert validate_class.validate(sample_data)['isSuccess'] is False
        assert 'event_type' in validate_class.validate(sample_data)['description']
